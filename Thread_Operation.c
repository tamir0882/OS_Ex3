...
DWORD WINAPI mission_thread(LPVOID lpParam)
{
	...
	//allocate memory for necessary pointers 
	...
	while (TRUE)
	{
		p_element = (Element*)malloc(sizeof(Element));
		if (NULL == p_element)
		{
			printf("create_queue: Memory allocation failure.\n");
			free(p_element);
			DestroyQueue(p_thread_data->q);
			return NULL;
		}


		wait_code = WaitForSingleObject(p_thread_data->h_resource_mutex, WAIT_TIME);
		if (WAIT_OBJECT_0 != wait_code)
		{
			printf("Thread operation failed - h_mission_mutex.wait() failed.\n");
			exit_code = FAILURE;

			ret_val = ReleaseMutex(p_thread_data->h_resource_mutex);
			if (0 == ret_val)
			{
				printf("Thread operation failed - h_mission_mutex.signal() failed.\n");
			}
			goto Resource_Handling;
		}

		p_thread_data->number_of_missions = p_thread_data->number_of_missions - 1;

		if (0 > p_thread_data->number_of_missions)
		{
			ret_val = ReleaseMutex(p_thread_data->h_resource_mutex);
			if (0 == ret_val)
			{
		...
		//read index from priority file.
		p_thread_data->next_line_offset = set_mission_index(p_thread_data->h_priority_file, p_element, p_thread_data->next_line_index);

		if (FAILURE == p_thread_data->next_line_offset)
		{
			printf("Thread operation failed - set_mission_index failed.\n");

			ret_val = ReleaseMutex(p_thread_data->h_resource_mutex);
			if (0 == ret_val)
			{
				printf("Thread operation failed - h_file_mutex.signal() failed.\n");
			}

			exit_code = FAILURE;
			goto Resource_Handling;
		}
		p_thread_data->next_line_index += p_thread_data->next_line_offset;


		Push(p_thread_data->q, p_element);
